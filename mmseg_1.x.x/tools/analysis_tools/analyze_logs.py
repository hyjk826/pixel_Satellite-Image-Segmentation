        ...
        for j, metric in enumerate(metrics):
            ...
            for epoch in epochs:
                ...
                if metric not in epoch_logs.keys():
                    continue
                if metric in ['mDice','mIoU', 'mAcc', 'aAcc']:
            ...
            if metric in ['mDice','mIoU', 'mAcc', 'aAcc']:
                ...
                plt.xlabel('step')
                if metric == 'mDice':
                    # Set a distinct line style, color, and marker for mDice
                    plt.plot(plot_epochs, plot_values, label=label, color='red', linestyle='-', marker='s', markersize=6)
                else:
